...
`loader` project, repository version 194:
    ...
    make -C src PLATFORM=c64 prg INSTALL=2000 RESIDENT=8f14 ZP=60 \
...
|---|---|---|
| `loader.prg` | `$8F14`–`$9000` | resident, 236 B. Flush under the map; boot KERNAL-loads it. |
...
`$8F14` is always RAM. `$01` must still be `BANK_LOADER` (`$35`) around
`jsr loadraw` (IEC `$DD00`), under `SEI`. GAME/HIGH loads must not cover `$8F14`.
