...
; $d018 = $06: screen block 0 ($C000), charset block 3 ($D800).

VIC_SCREEN = $c000
SPR_PTR = VIC_SCREEN + $3f8		; $c3f8
VIC_SPRITES = $c800
CHARSET = $d800
CHARSET_NUM = 106
...
SPRITE_HEAD = $800			; first 2K always-RAM; gfx copy_vic sends the full blob
SPRITE_TAIL = SPRITE_BYTES - SPRITE_HEAD	; $D000–$D7BF under I/O
D018_VIC = $06
...
PY_TAB = $ac00
SQTAB_BASE = $b800
!if PY_TAB + PY_TAB_SIZE != SQTAB_BASE {
	!error "py_tab must sit flush under SQTAB"
}
!if SQTAB_BASE + $800 != VIC_SCREEN {
	!error "SQTAB must sit flush under VIC screen"
}
...
; -DUSE_KRILL=1: native Krill (236 B at $C400). Default 0: KERNAL LOAD ($FFD5).
!ifndef USE_KRILL {
	USE_KRILL = 0
}

!if USE_KRILL {
	; Krill v194: resident $C400, install $2000, ZP $60. No KERNAL fallback.
...
LOCODE_BASE	= $0400			; MENU overlay, then game.prg ($0400–py_tab)
